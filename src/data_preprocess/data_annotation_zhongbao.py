...
def generate_file(out_file):
    ...
    global select_list
    info_json = []
    query_id_list = [i for i in range(0, query_num) if i in select_list]
    ...
    info_json = [item for item in info_json if len(item['doc_list']) >= 5]
    print(len(info_json))
    input()
...
select_list = []
with open('select_list.txt') as f:
    for line in f.readlines():
        select_list.append(int(line.strip()))

for i in range(0, 3):
    generate_file(out_path + 'id_' + str(i) + '.json')
